...
async function signInWithGoogle() {
    ...
    try {
        ...
        console.error("Lỗi đăng nhập hệ thống:", error);
        if (error.code === 'auth/popup-blocked') {
            alert("❌ Trình duyệt đã chặn cửa sổ Popup! Vui lòng mở khóa trên thanh địa chỉ.");
        } else if (error.code === 'auth/popup-closed-by-user') {
            console.log("Người dùng chủ động đóng popup.");
        } else {
...
function getLocalizedDate(match) {
    ...
    // Chuyển đổi định dạng YYYY-MM-DD sang ngày xem trực quan tiếng Việt
    if (dateStr.includes("-")) {
...
async function fetchWorldCupData() {
    ...
    try {
        ...
        data.matches.forEach((item, idx) => {
            ...
            // Phân loại Vòng Đấu động
            let type = "vong-bang";
            const roundLower = item.round.toLowerCase();
            ...
            // Đẩy vào mảng chuẩn của ứng dụng
            officialMatches.push({
                ...
                type: type,
                isHot: idx % 10 === 0, // Mock ngẫu nhiên trận hot
                result: (item.score1 !== null && item.score2 !== null) ? {
                    home: item.score1,
                    away: item.score2,
                    goals: []
                } : null
